...
(function(){
  console.log('Menjalankan versi dummy form koreksi...');
  ...
  const dummyRek = ['BCA - 123456 - TEST', 'BRI - 789012 - DEMO'];
  const mapRek = { 'BCA - 123456 - TEST': '123456', 'BRI - 789012 - DEMO': '789012' };
  const labelOptions = dummyRek.map(label => `<option value="${label}">${label}</option>`).join('');
  ...
  document.getElementById('submitKoreksi').onclick = function() {
    ...
    alert(`✅ Dummy Submit\nRekening: ${bank}\nJumlah: ${amt}\nDeskripsi: UANG MASUK DARI ${src} ${tgl}/${bln}`);
